...
| `Elio-VF03.woff2` | Elio's own variable font — 2 masters, Hair (wght 150) / Ultra (wght 850). Re-built 2026-09-09 with a rigorous per-glyph compatibility scan (the source `.glyphs` file, publicly renamed "Elio" 2026-08-31 but internally still called `Asta.glyphs`, is under active parallel work — Ultra is being filled in progressively). **104 glyphs actually ship in this build** (measured cmap) — every glyph whose Ultra master isn't drawn yet, or that references one that isn't, is explicitly excluded rather than silently broken: digits, most punctuation, currency symbols (cent/dollar/euro are the exception — rebuilt via proportional bar reconstruction, see below), and composites built on 4 still-Hair-only combining marks (dieresiscomb/tildecomb/ringcomb/cedillacomb — blocks Adieresis-type letters, Atilde-type, Aring/aring, Ccedilla/ccedilla). Fixed this round: a font-wide bug where every combining mark's attachment anchor sat at (0,0) instead of its own shape's edge, causing accented letters to render with the mark floating far too high; acutecomb/circumflexcomb are now the source masters with gravecomb/caroncomb built as mirrored components; 58 accented letters newly wired up for Ultra via anchor-based auto-alignment (36 of which are compatible enough to ship in this specific build, the rest await the 4 marks above). Built from `Asta.glyphs` via Glyphs' own `ExportDesignspaceAndUFO` + ufo2ft — glyphsLib's direct `.glyphs` parser errors on this file's format-4 node data, so the designspace/UFO route is required, not the raw-file route Norma's build script uses. Build script: `Projects/brands/August/00_Typeface/Asta/Export/compile_vf.py` (its `NOT_YET_IN_ULTRA` set is the authoritative exclusion list — re-scan for shape-count mismatches before bumping the version, since glyphs move out of it as Ultra gets drawn). | `src/app.html` @font-face |
