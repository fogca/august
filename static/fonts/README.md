...
| `Elio-VF05.woff2` | Elio's own variable font — 2 masters, Hair (wght 150) / Ultra (wght 850). Re-built 2026-09-05, same day as VF04: a follow-up re-export picked up `circumflexcomb`'s tightened sidebearing (114→107), which cascades to `caroncomb` (its mirrored component) and `Icircumflex`/`icircumflex` (composites) — a spacing refinement, not a compatibility change, so the exclusion set is unchanged from VF04. **106 glyphs ship** (measured cmap) — every glyph whose Ultra master isn't drawn yet, or that references one that isn't, is explicitly excluded rather than silently broken: digits, most punctuation (including `quotedblleft`/`quotedblright`, whose own Ultra shapes remain unfinished even though `quoteleft`/`quoteright` now ship as of VF04), currency symbols (cent/dollar/euro are the exception — rebuilt via proportional bar reconstruction), and composites built on 4 still-Hair-only combining marks (dieresiscomb/tildecomb/ringcomb/cedillacomb — blocks Adieresis-type letters, Atilde-type, Aring/aring, Ccedilla/ccedilla). The source `.glyphs` file is publicly renamed "Elio" (2026-08-31) but internally still called `Asta.glyphs`, under active parallel work — Ultra is being filled in progressively. Built from `Asta.glyphs` via Glyphs' own `ExportDesignspaceAndUFO` + ufo2ft — glyphsLib's direct `.glyphs` parser errors on this file's format-4 node data, so the designspace/UFO route is required, not the raw-file route Norma's build script uses. Build script: `Projects/brands/August/00_Typeface/Asta/Export/compile_vf.py` (its `NOT_YET_IN_ULTRA` set is regenerated by a live scan before every build — re-run it before bumping the version, since glyphs move out of it as Ultra gets drawn). | `src/app.html` @font-face |
